...
int displaygif(char *GIFFileName,u8 mode,u32 x,u32 y,int member,int denom)
{
	...
	gifbuffer=mymalloc(SRAMEX,GIFFile.obj.objsize);
...
void gifdisplay_demo(void)
{
	...
	while(1)
	{
		GUI_DispStringHCenterAt("在指定位置显示一张缩小1/2的加载到RAM中的GIF图片",400,0);
		displaygif("0:/PICTURE/GIF/火柴人.gif",0,20,20,1,2);
		GUI_Delay(1000);
		GUI_Clear();
	
		GUI_DispStringHCenterAt("在LCD中间显示一张扩大1倍的加载到RAM中的GIF图片",400,0);
		displaygif("0:/PICTURE/GIF/火柴人1.gif",1,0,0,2,1);	
		GUI_Delay(1000);
		GUI_Clear();
        
		GUI_DispStringHCenterAt("在LCD中间显示一张缩小1/2的加载到RAM中的GIF图片",400,0);
		displaygif("0:/PICTURE/GIF/龙.gif",1,0,0,1,2);
		GUI_Delay(1000);
		GUI_Clear();
		
		GUI_DispStringHCenterAt("在指定位置显示一张缩小1/2的无需加载的GIF图片",400,0);
		displaygifex("0:/PICTURE/GIF/火柴人.gif",0,20,20,1,2);
		GUI_Delay(1000);
		GUI_Clear();
		
		GUI_DispStringHCenterAt("在LCD中间显示一张扩大1倍的无需加载的GIF图片",400,0);
		displaygifex("0:/PICTURE/GIF/火柴人1.gif",1,0,0,2,1);	
		GUI_Delay(1000);
		GUI_Clear();
	
		GUI_DispStringHCenterAt("在LCD中间显示一张缩小1/2的无需加载的GIF图片",400,0);
		displaygifex("0:/PICTURE/GIF/龙.gif",1,0,0,1,2);
		GUI_Delay(1000);
		GUI_Clear();
	}
}
