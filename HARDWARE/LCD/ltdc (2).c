...
//根据不同的颜色格式,定义帧缓存数组
	...
	u32 ltdc_lcd_framebuf[LCD_WIDTH][LCD_HEIGHT] __attribute__((at(LCD_FRAME_BUF_ADDR)));	//定义最大屏分辨率时,LCD所需的帧缓存数组大小
	u32 sdram_array[LCD_WIDTH][LCD_HEIGHT] __attribute__((at(SDRAM_FRAME_BUF_ADDR)));	
	u32 Main_array[LCD_WIDTH][LCD_HEIGHT] __attribute__((at(MAIN_FRAME_BUF_ADDR)));	
#else
	u16 ltdc_lcd_framebuf[LCD_WIDTH][LCD_HEIGHT] __attribute__((at(LCD_FRAME_BUF_ADDR)));	//定义最大屏分辨率时,LCD所需的帧缓存数组大小
	u16 sdram_array[LCD_WIDTH][LCD_HEIGHT] __attribute__((at(SDRAM_FRAME_BUF_ADDR)));
	u16 Main_array[LCD_WIDTH][LCD_HEIGHT] __attribute__((at(MAIN_FRAME_BUF_ADDR)));		
#endif
...
/*******************************lcd扩展代码******************************************/	
void Enter_SDRAM(void)
{
	if(ltdc_framebuf[0]==(u32*)&ltdc_lcd_framebuf)
	{
		mymemcpy(sdram_array,ltdc_lcd_framebuf,sizeof(ltdc_lcd_framebuf));
		ltdc_framebuf[0]=(u32*)&sdram_array;
	}
	else if(ltdc_framebuf[0]==(u32*)&sdram_array)
	{
		mymemcpy(ltdc_lcd_framebuf,sdram_array,sizeof(sdram_array));
		ltdc_framebuf[0]=(u32*)&ltdc_lcd_framebuf;	
	}
}

void Quit_SDRAM(void)
{
	LTDC_Layer_Parameter_Config(0,(u32)ltdc_framebuf[0],LCD_PIXFORMAT,255,0,6,7,0X000000);//层参数配置
	LTDC_Layer_Window_Config(0,0,0,lcdltdc.pwidth,lcdltdc.pheight);	//层窗口配置,以LCD面板坐标系为基准,不要随便修改!
}

void Enter_Main_SDRAM(void)
{
	ltdc_framebuf[0]=(u32*)&Main_array;
}
void Quit_Main_SDRAM(void)
{
//	mymemcpy(sdram_array,Main_array,sizeof(Main_array));
//	ltdc_framebuf[0]=(u32*)&sdram_array;	
	if(pLayerCfg.FBStartAdress==(u32)ltdc_lcd_framebuf)
	{
		mymemcpy(sdram_array,Main_array,sizeof(Main_array));
		ltdc_framebuf[0]=(u32*)&sdram_array;
	}
	else if(pLayerCfg.FBStartAdress==(u32)&sdram_array)
	{
		mymemcpy(ltdc_lcd_framebuf,Main_array,sizeof(Main_array));
		ltdc_framebuf[0]=(u32*)&ltdc_lcd_framebuf;	
	}
	LTDC_Layer_Parameter_Config(0,(u32)ltdc_framebuf[0],LCD_PIXFORMAT,255,0,6,7,0X000000);//层参数配置
	LTDC_Layer_Window_Config(0,0,0,lcdltdc.pwidth,lcdltdc.pheight);	//层窗口配置,以LCD面板坐标系为基准,不要随便修改!
